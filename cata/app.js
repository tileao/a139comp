...
function absolutizeUrl(src, baseHref) {
  try { return src ? new URL(src, baseHref || location.href).href : src; } catch { return src; }
}

function normalizeAdcPayload(payload, baseHref) {
  if (!payload) return null;
  const out = JSON.parse(JSON.stringify(payload));
  if (out?.chart?.src) out.chart.src = absolutizeUrl(out.chart.src, baseHref);
  return out;
}

function readAdcPayloadFromIframe() {
  try {
    const bridge = adcFrame.contentWindow?.__adcBridge;
    const payload = bridge?.getPayload?.();
    return normalizeAdcPayload(payload, adcFrame.contentWindow?.location?.href);
  } catch {
    return null;
  }
}

function hasUsableAdcPayload(payload) {
  return !!(payload?.chart?.src && Array.isArray(payload?.analysis?.rows) && payload.analysis.rows.length);
}

function syncAdcPreviewStateFromIframe() {
  const payload = readAdcPayloadFromIframe();
  if (hasUsableAdcPayload(payload)) adcPreviewState.payload = payload;
  return adcPreviewState.payload;
}

function parseAdcDecisionRowsFromDoc(doc) {
  return [...(doc?.querySelectorAll?.('#decisionTable tr') || [])].map(tr => {
    const tds = tr.querySelectorAll('td');
    if (tds.length < 3) return null;
    const point = (tds[0]?.textContent || '').trim();
    const asdaText = (tds[1]?.textContent || '').trim();
    const verdict = (tds[2]?.textContent || '').trim();
    const go = /^OK$/i.test(verdict);
    const availableAsda = numberFromText(asdaText) || 0;
    return {
      id: /^(full|pista|full length|pav|thr)/i.test(point) ? 'FULL' : point,
      name: point,
      point,
      availableAsda,
      availableMeters: availableAsda,
      go,
      rtoOk: go
    };
  }).filter(Boolean);
...
function buildAdcFallbackPayload(doc) {
  try {
    const win = adcFrame.contentWindow;
    const bridge = win?.__adcBridge;
    const state = bridge?.getCurrentState?.() || {};
    const chart = bridge?.getChartSource?.() || null;
    const rows = parseAdcDecisionRowsFromDoc(doc);
    if (!chart?.src || !rows.length) return null;
    return normalizeAdcPayload({
      baseId: state.currentBaseId || doc.getElementById('baseSelect')?.value || '',
      runwayId: state.currentRunwayId || '',
      departureEnd: state.departureEnd || '',
      vizPage: state.vizPage || 'P1',
      rto: Number(doc.getElementById('rtoInput')?.value || 0),
      chart,
      runway: null,
      analysis: { rows, basisMetric: 'ASDA', meta: { startLabel: state.departureEnd || '' } }
    }, win?.location?.href);
  } catch {
    return null;
  }
...
async function runADC(input, rtoResult) {
  ...
  if (bridge?.analyzeFromBridge) {
    try {
      const payload = await bridge.analyzeFromBridge(request);
      finalPayload = hasUsableAdcPayload(payload) ? normalizeAdcPayload(payload, adcFrame.contentWindow?.location?.href) : null;
      if (!finalPayload) {
        finalPayload = await waitForTruthy(() => {
          const p = readAdcPayloadFromIframe();
          return hasUsableAdcPayload(p) ? p : null;
        }, 3000);
      }
    } catch (error) {
      console.warn('ADC bridge falhou, tentando fallback DOM', error);
    }
  }
  ...
  if (!finalPayload) {
    ...
    try {
      if (doc.defaultView?.setVizPage) doc.defaultView.setVizPage('P1');
    } catch {}
    await sleep(90);
    try { doc.defaultView?.analyze?.(); } catch { clickField(doc, 'analyzeBtn'); }
    await waitForTruthy(() => doc.querySelectorAll('#decisionTable tr').length > 0, 5000);
    finalPayload = readAdcPayloadFromIframe() || buildAdcFallbackPayload(doc);
  }

  if (hasUsableAdcPayload(finalPayload)) {
    adcPreviewState.payload = finalPayload;
    const rows = (finalPayload?.analysis?.rows || []).map(row => ({
      id: row.id || '',
      point: row.name || row.point || '',
      rtoOk: row.rtoOk ? 'OK' : 'NO',
      decision: row.go ? 'PODE' : 'NO GO',
      go: !!row.go,
      availableAsda: Number(row.availableAsda || 0),
      availableMeters: Number(row.availableAsda || row.availableMeters || 0)
    }));
    const fullRow = rows.find(row => row.id === 'FULL') || rows[0] || null;
    return {
  ...
  const rows = parseAdcDecisionRowsFromDoc(doc).map(row => ({
    id: row.id,
    point: row.point,
    rtoOk: row.rtoOk ? 'OK' : 'NO',
    decision: row.go ? 'PODE' : 'NO GO',
    go: !!row.go,
    availableAsda: row.availableAsda,
    availableMeters: row.availableMeters
  }));
  const fullRow = rows.find(row => row.id === 'FULL') || rows[0] || null;
  return {
