...
(function () {
  ...
  function renderTable(results, criticalIndex, watMax) {
    var tbody = document.getElementById('flightTableBody');
    var watHeader = document.getElementById('watMarginHeader');
    var showWat = watMax !== null && watMax !== undefined;
    if (watHeader) watHeader.hidden = !showWat;
    tbody.innerHTML = '';
    results.forEach(function (r, i) {
      ...
      if (showWat) {
        var wm = r.watMargin;
        var wmKnown = wm !== null && wm !== undefined && isFinite(wm);
        var wmCls = wmKnown && wm < 0 ? 'wat-margin-bad' : wmKnown && wm < 200 ? 'wat-margin-warn' : 'wat-margin-ok';
        watCell = '<td class="' + (wmKnown ? wmCls : '') + '">' + (wmKnown ? (wm >= 0 ? '+' : '') + fmt(wm) : '—') + '</td>';
      }
      tr.innerHTML =
